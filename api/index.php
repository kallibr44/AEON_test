...
if ($path == 'auth.sendCode') call('POST', $method, $query, 'Session::phone_code');
    ...
    // routes (users)
    if ($path == 'users.get') call('GET', $method, NULL, 'User::owner_info');
    if ($path == 'users.update') call('POST',$method, $query, "User::owner_update");
    // routes (notifications)
    if ($path == 'notifications.get') call('GET',$method,$query,"Notification::get_user_notifications");
    if ($path == 'notifications.read') call('GET',$method, NULL, "Notification::read_notifications");
    // routes (not found)
